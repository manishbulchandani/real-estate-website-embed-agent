// ==UserScript==
// @name         Akshar Developers Chatbot Embed
// @namespace    http://tampermonkey.net/
// @version      1.0
// @description  Automatically embeds the real estate chatbot on Akshar Developers website
// @author       Antigravity
// @match        https://www.akshardevelopers.com/*
// @grant        none
// ==/UserScript==
...
(function () {
    ...
    function shouldOpenChatbotFromUrl() {
        try {
            return new URL(window.location.href).searchParams.get('chatbot') === 'open';
        } catch (error) {
            return false;
        }
    }
    ...
    window.addEventListener('message', (event) => {
        ...
        if (!data || data.type !== 'chatbot:navigate' || !data.url) {
            return;
        }

        try {
            const targetUrl = new URL(String(data.url), window.location.href);
            targetUrl.searchParams.set('chatbot', 'open');
    ...
    console.log('%c Chatbot Embed Loaded via Tampermonkey! ', 'background: #6495ce; color: #fff; padding: 5px; border-radius: 4px;');
})();
